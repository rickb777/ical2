...
func ExampleVEventWithTimezone() {
	...
	event := &ical2.VEvent{
		...
		Attendee:     []value.CalAddressValue{value.CalAddress("ann.blin@example.com").With(parameter.Role(parameter.REQ_PARTICIPANT), parameter.CommonName("Ann Blin"))},
...
func ExampleMeetingVEvent() {
	...
	event := &ical2.VEvent{
		...
		End:          value.TStamp(de),
		Organizer:    value.CalAddress("ht@throne.com").With(parameter.CommonName("H.Tudwr")),
		Attendee:     []value.CalAddressValue{cath1, ann1, jane, ann2, cath2, cath3},
